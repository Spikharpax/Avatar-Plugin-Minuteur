...
var start = function (speakOnClient, client, time, type, sound, addspeech) {
	...
	if (time instanceof Date && !isNaN(time.valueOf())) {
		...
		if (moment(timerTime).isAfter(maxDate) == true) {
			return Avatar.speak("Je ne peux pas programmer une date à plus de 6 jours", speakOnClient, function () {
...
var timer_done = function (speakOnClient, client, sound, addspeech, itemTimer, hms) {
	...
	// Client mappé avec server speak
	...
	if (!Avatar.Socket.isServerSpeak(client) && !serverSpeak) {
			...
			Avatar.play('%TRANSFERT%/'+sound, client, function(){
				...
				if (addspeech) {
					...
					Avatar.speak(tts, client, function() {
						let ttsExportPlugin = Config.modules.timer.ttsPluginExport;
						if (ttsExportPlugin && Avatar.exists(ttsExportPlugin)) {
		...
		let ttsExportPlugin = Config.modules.timer.ttsPluginExport;
		if (ttsExportPlugin && Avatar.exists(ttsExportPlugin)) {
				// apres @@ c'est le répertoire partagé Sonos avec le fichier
				// l'avantage de faire ca est que on peut avoir un répertoire n'importe où
				// et pas seulement dans le plugin
			  var folder = path.normalize(__dirname)+'@@'+'/sound/' + sound;
			 	Avatar.play(folder, client, function(){
					if (addspeech) {
						var tts;
						if (itemTimer && Config.modules.timer.times[itemTimer].speech)
							tts = Config.modules.timer.times[itemTimer].speech;
				...
				error('Minuteur: Aucun plugin n\'est défini pour déporter le son');
			}
		}
}
